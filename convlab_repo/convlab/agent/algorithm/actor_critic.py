...
class ActorCritic(Reinforce):
    ...
    def init_nets(self, global_nets=None):
        ...
        self.reward_agent = reward_agent.RewardAgent_EncoderSide(config, use_gpu, "mine")
    ...
    def calc_pdparam_v(self, batch):
        ...
        Args:
            batch:
        Returns:
            pdaram: logits for discrete prob from one nets.
            v_pred: value_predict from critic
        """
    ...
    def fetch_irl_reward(self, batch):
        self.reward_agent.eval()
        with torch.no_grad():        
            reward = self.reward_agent(copy.deepcopy(batch), surgery = "das").detach().view(-1)
