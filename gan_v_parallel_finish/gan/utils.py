...
def save_model(agent, config):
    ...
    torch.save({
                ...
                os.path.join(config.session_dir, "model_lirl.mdl"))
    logger.info("Model Saved.")

def save_model_woz(agent, config, training_mode = "000"):
    # saved two model over here.
    # one is common model,
    # one is model in that training mode.
    ...
    torch.save({"generator":agent.generator.state_dict(),
                ...
                os.path.join(config.session_dir, "model_lirl_" + training_mode))


def save_model_vae(agent, config, name = None):
    if not name:
        torch.save({"vae":agent.vae.state_dict()},
                    os.path.join(config.session_dir, "model_vae"))
    else:
        torch.save({"vae":agent.vae.state_dict()},
                    os.path.join(config.session_dir, "model_vae_" + str(name)))
    # logger.info("Model Saved.")
